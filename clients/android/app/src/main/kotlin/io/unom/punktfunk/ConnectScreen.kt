...
fun ConnectScreen(
    ...
    fun doConnect(
        ...
        if (settings.autoWakeEnabled && macs.isNotEmpty() && down) {
            ...
            doConnectDirect(targetHost, targetPort, name, pinHex, profile, launch, onMismatch = onMismatch, onFailure = {
                waker.start(
                    hostName = name,
                    connectsAfter = true,
                    macs = macs,
                    ...
                    onOnline = {
                        ...
                        doConnectDirect(
                            ...
                            profile, launch, onMismatch = onMismatch,
                        )
                    },
                )
            })
        } else {
            doConnectDirect(targetHost, targetPort, name, pinHex, profile, launch, onMismatch = onMismatch)
        }
    }
    ...
    fun connect(
        ...
        when {
            ...
            known != null && known.fpHex.isNotEmpty() -> doConnect(
                targetHost, targetPort, known.name, known.fpHex, oneOffProfile, launch,
                onMismatch = if (saved == null && dh == null) {
                    {
                        pendingTrust = PendingTrust(
                            targetHost, targetPort, typed ?: targetHost, null,
                            PendingTrust.Kind.FP_CHANGED, oneOffProfile, launch,
                        )
                    }
                } else {
                    null
        ...
        when (val resolved = DeepLinks.resolveHost(link, savedHosts)) {
            ...
            is HostResolution.Record -> {
                ...
                connect(
                    ...
                    oneOffProfile = profileRef, launch = link.launch, saved = resolved.host,
                )
            }
    ...
    ConnectGrid(
        ...
        onConnect = { kh, oneOff -> connect(kh.address, kh.port, oneOffProfile = oneOff, saved = kh) },
    ...
    ConnectPrompts(
        ...
        onConfirmLinkConnect = { plc ->
            ...
            connect(
                ...
                oneOffProfile = plc.profile, launch = plc.launch, saved = plc.host,
            )
        },
