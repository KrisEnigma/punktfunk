...
test("capture mode status accepts only command results", () => {
  assert.equal(Model.captureModeStatus("dedicated\n"), "dedicated")
  assert.equal(Model.captureModeStatus(" mirror "), "mirror")
  assert.equal(Model.captureModeStatus("DP-2"), null)
  assert.equal(Model.captureModeStatus(""), null)
})
...
test("pairing window failure clears its complete lifecycle state", () => {
  const state = Model.pairingWindow({
    armed: true,
    pin: "1234",
    pin_pending: true,
  ...
  assert.deepEqual(state, {
    armed: true,
    pairingPin: "1234",
    pinDevices: [{ uniqueid: "moonlight" }],
    pinPending: true
  })
  Object.assign(state, Model.pairingWindow(null))
  assert.deepEqual(state, {
    armed: false,
    pairingPin: "",
    pinDevices: [],
...
test("pairing rows carry each PIN ceremony before the approval queue", () => {
  const rows = Model.pairingRows(
    true,
    [{ uniqueid: "moonlight", fingerprint: "abc", peer_ip: "192.0.2.1" }],
    [{ id: 3, name: "tv" }]
  )
  assert.deepEqual(rows.map((r) => r.kind), ["arm", "pin", "pending"])
  assert.equal(rows[1].device.uniqueid, "moonlight")
  assert.equal(rows[2].device.id, 3)
  assert.deepEqual(Model.pairingRows(false, [], []), [])
})

test("pinArgs addresses the PIN to its waiting ceremony", () => {
  assert.deepEqual(
    Model.pinArgs("1234", {
      uniqueid: "moonlight",
      fingerprint: "abc",
    ...
    ["pin", "1234", "moonlight", "abc", "192.0.2.1"]
  )
  assert.deepEqual(Model.pinArgs("", { uniqueid: "moonlight" }), [])
  assert.deepEqual(Model.pinArgs("1234", null), [])
})
...
test("tabs wrap and name a waiting pair count", () => {
  assert.equal(Model.tabId(0), "overview")
  assert.equal(Model.tabId(4), "stats")
  assert.equal(Model.nextTab(0, -1), 4)
  assert.equal(Model.nextTab(4, 1), 0)
  assert.equal(Model.tabLabel("pair", 2), "Pair · 2")
  assert.equal(Model.tabLabel("stats", 0), "Stats")
})

test("incoming pair is the first pending device", () => {
  assert.equal(Model.incomingPair([]), null)
  assert.equal(Model.incomingPair([{ id: 3 }]).id, 3)
  assert.equal(Model.remainingPair([{ id: 3 }, { id: 4 }])[0].id, 4)
})

test("policy rows follow the effective display object", () => {
  assert.deepEqual(Model.policyRows({}), [])
  const rows = Model.policyRows({
    topology: "extend",
    identity: "client",
    mode_conflict: "prefer-client",
    max_displays: 2
  })
  assert.equal(rows[0].v, "extend")
  assert.equal(rows[3].v, "2")
})

test("stats pillars use the encoder target and encode p99", () => {
  assert.deepEqual(Model.statsPillars({}), [])
  const pillars = Model.statsPillars({
  ...
  assert.equal(pillars[0].text, "40.0")
  assert.equal(pillars[1].text, "118")
  assert.equal(pillars[2].text, "2.3")
  assert.equal(Model.chartSeries(false).length, 1)
  assert.equal(Model.chartSeries(true)[3].key, "encode")
})
