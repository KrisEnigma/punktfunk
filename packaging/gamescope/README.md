...
| `0021-headless-change-the-virtual-display-s-mode-at-runtim.patch` | `GAMESCOPE_SET_OUTPUT_MODE` (three root cardinals: width, height, refresh in mHz) re-resolves a headless output's mode while the session runs — output globals, the connector's mode and refresh lists, the nested resolution when it was derived from the output, and a framerate cap still equal to the refresh being left. `GAMESCOPE_OUTPUT_MODE_FEEDBACK` reports the mode in effect. Every other backend refuses through `IBackend`'s default | **Yes** — a virtual display whose mode the caller chose is the one display that can change it without hardware |
...
| `+pfhdr17` | …and `GAMESCOPE_SET_OUTPUT_MODE` changes a headless output's mode while the session runs, so one kept compositor serves any client mode |
