...
export type NativeClient = { readonly "access_level"?: string | null, readonly "expires_unix"?: never, readonly "fingerprint": string, readonly "granted_unix"?: never, readonly "grants"?: never, readonly "name": string, readonly "until_disconnect": boolean }
export const NativeClient = Schema.Struct({ "access_level": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "`full` | `controller` | `view` | `custom`. Derived from `grants`;\nabsent only on hosts older than this field." })), "expires_unix": Schema.optionalKey(Schema.Never), "fingerprint": Schema.String.annotate({ "description": "Hex SHA-256 of the client certificate — the stable id." }), "granted_unix": Schema.optionalKey(Schema.Never), "grants": Schema.optionalKey(Schema.Never), "name": Schema.String, "until_disconnect": Schema.Boolean.annotate({ "description": "The record is dropped when the device's last session ends, rather than at a clock\ntime. `expires_unix` may also be set; whichever comes first ends the grant." }) })
