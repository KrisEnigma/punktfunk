...
export const SessionAudioRequest = Schema.Struct({ "muted": Schema.Boolean })
...
export type SessionRow = { readonly "client": string, readonly "client_name"?: string | null, readonly "fps": number, readonly "height": number, readonly "id": number, readonly "joined": boolean, readonly "muted": boolean, readonly "width": number }
export const SessionRow = Schema.Struct({ "client": Schema.String.annotate({ "description": "12-hex cert-fingerprint prefix, or the peer IP if anonymous." }), "client_name": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "Display name (trust-store, else the client's own). Absent if nameless." })), "fps": Schema.Number.annotate({ "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), "height": Schema.Number.annotate({ "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), "id": Schema.Number.annotate({ "description": "Registry id: the `{id}` in `PUT /session/{id}/audio`. Not stable across host restarts.", "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), "joined": Schema.Boolean.annotate({ "description": "Admitted by `mode_conflict: join`: shares the owner's display and audio." }), "muted": Schema.Boolean.annotate({ "description": "Audio is silence on the wire: the operator's mute or the title's `audio.sessions`." }), "width": Schema.Number.annotate({ "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)) }).annotate({ "description": "One live native session, as the per-session routes address it." })
