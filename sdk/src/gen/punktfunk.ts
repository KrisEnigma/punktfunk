...
export const StageTiming = Schema.Struct({ "name": Schema.String.annotate({ "description": "Pipeline order, named per path. Linux native: `queue capture submit encode send`.\nWindows driver: `driver copy send`. GameStream: `capture encode packetize send\nsend_spread`, or `driver copy send send_spread` on the Windows driver." }), "p50_us": Schema.Number.annotate({ "format": "float" }).check(Schema.isFinite()), "p99_us": Schema.Number.annotate({ "format": "float" }).check(Schema.isFinite()) }).annotate({ "description": "One stage's p50/p99 in an aggregation window (microseconds)." })
