...
regenerates the spec and diffs it against the committed file, the `docs-drift` job checks that
`docs-site/public/openapi.json` — the snapshot the docs site serves — is a byte-for-byte copy of it,
and `sdk-plugin-kit` regenerates the SDK's TypeScript client from it. Touch the management API and
CI stays red until you regenerate all three:
