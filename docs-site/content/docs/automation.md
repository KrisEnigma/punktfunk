...
| `session.started` / `session.ended` | an A/V session registers / ends | session id, client label, mode (`3840x2160@120`), HDR. `session.ended` adds a `summary`: duration, codec / bit depth / chroma, bitrate, frames sent and dropped, bring-up ms, path MTU, and `ended` — `local`, `game_exited`, `host_ended`, `host_error`, `lost` or `stopped_by_operator`. The same shape `GET /api/v1/session/last` returns for the last eight |
