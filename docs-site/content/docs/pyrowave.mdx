...
Packet framing and FEC add ~15 % on top of these rates, and Gigabit Ethernet tops out around
940 Mbps of payload, so 4K60 wants 2.5GbE even at 4:2:0 and the big 4:4:4 / HDR / high-refresh
modes want 5GbE or 10GbE. **Do not run this over Wi-Fi** — that's
...
5.3 Gbps, which is more than a 5GbE link carries. A client bitrate does not lower it: PyroWave
ignores it. On a link that can't keep up, lower the mode or cap the host's pin with
...
2. **Client**:
   ...
   - Android (phone, tablet, TV box — wired Ethernet strongly recommended): set
     **Settings → Video codec → PyroWave (wired LAN)**. The row appears only on a 64-bit device
     whose GPU passes the decode probe: Vulkan 1.3 with the codec's compute feature set, which
     recent Adreno and Mali parts have and older ones do not. This one codec decodes on the GPU
     and presents through its own swapchain rather than through the hardware video decoder, so
     the low-latency and presentation settings that tune MediaCodec do not apply to it.
3. Leave the bitrate on Automatic: a PyroWave session pins itself to the ~1.6 bpp rate for
   your mode (≈200 Mbps at 1080p60; ~2.6 bpp for 4:4:4, +15 % for HDR). A client bitrate is
   ignored, and the adaptive-bitrate controller stays off — this codec has no useful low-rate
...
- Linux and Windows hosts; Linux clients (the GTK desktop app and the session client, including
  a docked Deck), the Windows client on x64, Apple clients (native Metal decode on Mac, and on
  A13-or-newer iPhone, iPad and Apple TV hardware), and 64-bit Android devices with a Vulkan 1.3
  GPU. The 32-bit Android build carries no decoder, and Wi-Fi cannot carry these rates on any
  client — this is a wired-LAN codec everywhere.
