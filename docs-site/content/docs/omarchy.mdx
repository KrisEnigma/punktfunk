...
is the remote desktop on this box — leave Sunshine off unless you are migrating. Both services
default to TCP 47990 even with GameStream off; see [Sunshine on the same
box](#sunshine-on-the-same-box) before you run them together.
</Callout>
...
`--status` reads local config only: it checks `PUNKTFUNK_CAPTURE_MONITOR` in `host.env` before
`display-settings.json`, and that override wins. The menu's checked rows do not start the host.
This screen is the [Streamed screen](/docs/virtual-displays#stream-a-real-monitor-instead) pin;
...
- **The management ports collide.** Sunshine's web UI and Punktfunk's management API both default
  to TCP 47990. The installer moves ours to 47991 when Sunshine is running or enabled, and
  `punktfunk-omarchy setup` opens the moved port in ufw. A port you
  [move by hand](/docs/configuration) opens on the next `punktfunk-omarchy setup`.
- **The GameStream plane collides.** Moonlight compatibility uses 47984/47989/48010 and
  47998/47999/48000 — exactly Sunshine's TCP and UDP ports. Ours are **off by default**; leave them
  off unless you are migrating, and only one host can bind them.
...
`remove` reverses every step of `setup` — units, drop-in, ufw rules, certificate trust, the webapp,
the bar widget, the hooks and the theme template — and restores both the screen-share picker and
the tray icon. Your config and pairings survive both; see [Uninstall](/docs/uninstall) to clear
those too.
