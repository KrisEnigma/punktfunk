...
pin; Dedicated clears it. Changing the mode restarts a running host, ends its active stream,
and applies to the next connection.
...
**Tailscale later triggers a rule update.** Setup opens `tailscale0` only while the iface exists.
A user path unit watches `/sys/class/net/tailscale0` and tries the same tagged rules when the
iface appears after setup. The unit cannot answer a `sudo` password prompt; if the rule is
missing, run `punktfunk-omarchy ufw` in a terminal. `punktfunk-omarchy remove` disables the watch.
...
- **The management ports collide.** Sunshine's web UI and Punktfunk's management API both default
  to TCP 47990. Stop Sunshine, or [move Punktfunk's management API](/docs/configuration) before
  starting both.
