...
| **App menu** | installs "Punktfunk Web Console" as an Omarchy webapp — it appears in Apps (`Super`+`Space`) and opens the secure login page in its own window. |
| **Bar widget** | the Punktfunk Quickshell widget in the bar — live status, pairing approvals and devices without opening the console. It replaces the tray icon, which is hidden while the widget is enabled. |
...
webapp, the bar widget, the hooks, the theme template — and restores both the screen-share picker
and the tray icon. Your config and pairings survive both; see
