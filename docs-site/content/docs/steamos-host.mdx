...
- The first build downloads a container image + toolchain (~1 GB) and takes ~10–15 minutes, then
  ~4 minutes for the FFmpeg the host carries, then another ~5–10 minutes for the HDR gamescope
  build (a second set of dependencies plus a fresh gamescope clone). Budget around 30 minutes for
  a first install. Later rebuilds are incremental,
