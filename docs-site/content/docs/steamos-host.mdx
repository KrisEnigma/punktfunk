...
glibc — the binary then runs natively on SteamOS) and wires it up as systemd user services.
...
Deck's AMD GPU (with **VAAPI** as the fallback), **NVENC** on NVIDIA. The installer writes
...
`PUNKTFUNK_VULKAN_ENCODE=0` in `host.env` to force the VAAPI path.
...
- The first build downloads a container image + toolchain (~1 GB) and takes ~15 minutes, then
  another ~5 minutes for the HDR gamescope build (a second set of dependencies plus a fresh
  gamescope clone). Budget around 20 minutes for a first install. Later rebuilds are incremental,
...
**One command instead?** The [guided installer](/docs/install#guided-install-preview) (preview) runs
exactly the two lines above and asks the two questions they have no flag for (Moonlight compat, the
shared clipboard). It does not ask about groups, linger or starting the services — the script below
always does those:
...
A source build has no package feed to be behind, so the card compares your checkout with `main`:
the button appears when there are commits to pull, and the version it reports carries the commit it
was built from (`0.35.0+g1a2b3c4`), which is how one rebuild is told from the next.
