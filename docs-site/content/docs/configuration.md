...
| `PUNKTFUNK_GAMESCOPE_VRR` | `1` · `0` *(default `1` = on)* | Adaptive sync for gamescope sessions. A headless gamescope normally composites on a synthetic vblank at the session rate, so a frame the game finishes just after the tick waits out a full period — and the stream carries measurably fewer unique frames than the session rate (the shortfall `PUNKTFUNK_VDISPLAY_HZ_MULT` works around at double the render cost). With the `punktfunk-gamescope` build (`+pfhdr9`) the session is spawned with `--adaptive-sync` + `--framerate-limit` at the game rate: gamescope paints — and the stream receives — every frame the moment the game presents it, and the limiter keeps the game paced at the session rate now that vblank no longer does. `0` restores the fixed-tick behaviour. Ignored on a stock or older gamescope. |
