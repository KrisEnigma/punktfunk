...
| `PUNKTFUNK_UI_BIND` | address *(default: `0.0.0.0`)* | Where the web console listens — `0.0.0.0` for your network, `127.0.0.1` for this machine only, or one address such as a VPN interface. On any bind the console answers only peers on the local network or a VPN (private and link-local addresses, IPv6 unique-local, Tailscale's `100.64.0.0/10`) and refuses the internet. The plugin-UI origin (`PUNKTFUNK_UI_PLUGIN_PORT`, below) always follows it, so 47993 is never wider than 47992. The port stays `PORT` / the console's own 47992. The guided installer asks for this and writes it here; `punktfunk-setup --web-bind=localhost` and, on Windows, `punktfunk-host service install --web-bind=127.0.0.1` set it without the question. On NixOS the equivalent is `services.punktfunk.web.bind`, and `host.env` is not read by the console. |
