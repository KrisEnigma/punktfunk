...
- **Pinned cards** — a host with pinned [settings profiles](/docs/client-settings) shows them
  ...
  doesn't create them.
- **Stream from a game's own page** — when a paired host has a Steam game in its
  [library](/docs/game-library) (the host's Steam library plugin is connected), that game's page
  on the Deck gets a **Stream** button in the play bar, next to Steam's own buttons. Tap it and
  the host launches the game into a stream — the same idea as Remote Play's **Stream** button,
