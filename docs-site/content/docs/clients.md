...
[Connect with Moonlight](/docs/moonlight). It gets encrypted video, error correction and
host-side adaptation to a lossy link like the native clients do; what it doesn't get are the
native protocol's extensions (client-side speed test, jumbo frames).
