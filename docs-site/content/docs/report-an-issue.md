...
A good report is one file with the host's log, its health checks and your client's log. Two clicks
build it.
...
newest 4096 lines at debug detail. If the upload fails, the host's TCP port **47990** is blocked —
see [Ports](/docs/ports), or grab the log [by hand](#logs-without-the-console).
...
checks, the whole host and plugin log, and every client log the host holds.
...
| Linux client | Start it from a terminal; it logs to standard output |
